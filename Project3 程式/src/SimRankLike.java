...
public class SimRankLike {
	private Graph<Integer, Edge> graph;
	private HashMap<Point,Double> scoreMap;
	private double dampingFactor;
	public SimRankLike(Graph<Integer, Edge> g, double c) {
		graph = g;
		dampingFactor = c;
		scoreMap = new HashMap<Point,Double>();
	}
	public SimRankLike(Graph<Integer, Edge> g) {
		this(g,0.8);
	}
	...
	public HashMap<Point,Double> runScore() {
		...
		do {
			...
			for(int j = i + 1 ; j < vertexNum ; j++) {
				...
				Set<Edge> incomingTempEdgeA = graph.incomingEdgesOf(tempA);
				Set<Edge> incomingTempEdgeB = graph.incomingEdgesOf(tempB);
				HashSet<Edge> incomingEdgeA = new HashSet<Edge>(incomingTempEdgeA);
				HashSet<Edge> incomingEdgeB = new HashSet<Edge>(incomingTempEdgeB);
				
				double tempScore = 0;
				if(incomingEdgeA.size() == 0 || incomingEdgeB.size() == 0) {
					tempScore = 0;
				}else {
					incomingEdgeA.add(new Edge(tempA,tempA, null, null));
					incomingEdgeB.add(new Edge(tempB,tempB, null, null));
					...
					for(Edge eB : incomingEdgeB) {
						...
						if((vertexFromA == tempA || vertexFromA == tempB)  && (vertexFromB == tempA || vertexFromB == tempB)) {
							continue;
						}
					...
					tempScore = dampingFactor / (incomingEdgeA.size() * incomingEdgeB.size() -1) * tempScore;
				}
		...
		System.out.println("SimRankLike is finished after " + step + " steps.");
		return scoreMap;
	}
